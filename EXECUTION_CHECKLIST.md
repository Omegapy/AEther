...
The minimal stable-sign obstruction has now been recorded, the first nonminimal derivative-mixing branch has been tested and found degenerate, the constraint-assisted longitudinal `U`-sector family has also been ruled out as a healthy nondegenerate branch under the active positive-sign standard, and the higher-derivative quartic branch has now been carried through a broader admissible-background theorem, an obstruction note, a first negative replacement-control test, a dedicated nonlinear-stability setup note, an explicit-completion reduced-system note, a first local well-posedness/continuation note, a coercive longer-time bootstrap note, a strict elliptic-assumption relaxation test, a strict-branch decay/integrability test, a first post-obstruction renormalized same-branch asymptotic test, the failure note for that recorded first-pass scalar-variable repair, a gauge-first unit-lapse spatial-harmonic reduced-system note on the same fixed completion, and now a first follow-on obstruction note for that unit-lapse formulation. The immediate substantive research step is therefore no longer to ask whether the old lapse obstruction survives. It does not. The next major burden is to repair the new fixed-completion trace/auxiliary package after the recorded trace-driven longitudinal Coulomb tail, and only after that smaller route fails should a deeper completion change be considered.

1. Decide whether the fixed-completion unit-lapse trace/auxiliary package should be repaired first by a propagated trace-monopole cancellation, a renormalized auxiliary Coulomb profile, or a weighted/homogeneous function class.
2. After choosing that smaller repair, determine whether the fixed-completion unit-lapse spatial-harmonic formulation then supports a first local well-posedness and continuation argument and a coercive/decay architecture.
