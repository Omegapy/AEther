...
\begin{proof}
Proposition \ref{prop:basis} fixes the carrier-slot structure. Proposition \ref{prop:EL} then shows that the normalized readout \(\nu\,\CurvSeed^{-1}\CurvOne\) cancels the bridge-curvature seed and leaves the coefficient triple \((\mathfrak c_I,\mathfrak c_\chi,\mathfrak c_W)\) unchanged. Corollaries \ref{cor:currentflux}, \ref{cor:continuity}, and \ref{cor:coulomb}, together with Proposition \ref{prop:pullback} and Theorem \ref{thm:gains}, fix the eliminated downstream package.
