...
The immediate research objective is now to work from a fixed explicit completion and reduced system rather than from an open-ended setup question. The broader admissible-background theorem, the obstruction note, the summable-patch-control negative test, the nonlinear-stability setup note, and the explicit-completion reduced-system note now all exist. The quartic branch therefore has a disciplined theorem endpoint, a disciplined first stability target, and one definite nonlinear branch on which that target can now be tested.

The next major burden is now specific: prove the first local well-posedness and continuation criterion for the explicit minimal quartic-compatible completion already chosen, including slice-wise control of the auxiliary elliptic constraints and persistence of the positivity/gap assumptions.
...
- [ ] Prove the slice-wise elliptic control and propagation needed for the auxiliary \(Q\)- and \(U\)-sector constraints of the explicit minimal completion.
...
- [ ] Decide whether the explicit-completion energy bookkeeping \(\mathcal E_N^{\mathrm{min},4}\) can be upgraded from a schematic control quantity to a coercive high-order estimate for the chosen completion.
- [ ] Confront the broader nonlinear stability problem on the surviving branch only after those first local-in-time setup tasks are completed.
