...
- do describe the current nonlinear-stability and post-stability route as centered on the explicit minimal quartic-compatible completion, its first local theorem, its coercive longer-time bootstrap, its strict elliptic-assumption relaxation test, the gauge-first unit-lapse reformulation, the recorded unit-lapse trace/auxiliary obstruction, the two failed smallest repair tests, the now-recorded weighted/homogeneous local-closure step, the now-recorded weighted/homogeneous coercive-bootstrap step, the now-recorded weighted/homogeneous decay/integrability step, the now-recorded same-class asymptotic/nonlinear-stability closure result, the now-recorded derivational bridge-closure test result, and the now-recorded explicit bridge-tensor reduction result, while still not treating substrate derivation as already completed
...
- the next substantive research step is now post-verdict: write the next-branch or redesign-criteria note that records the quartic branch as stable but derivationally nonclosing and fixes the smallest admissible requirements for the next candidate bridge/completion beyond it
